...
class PGAgent(BaseAgent):
    ...
    def calculate_q_vals(self, rewards_list):
        ...
        # Estimate Q^{pi}(s_t, a_t) by the discounted sum of rewards starting from t
            ...
            for reward in rewards_list:
                q_values.append(self._discounted_cumsum(reward))
    ...
    def estimate_advantage(self, obs, rews_list, q_values, terminals):
        ...
        if self.nn_baseline:
            ...
            values = (values_unnormalized-np.mean(values_unnormalized)+np.mean(q_values))/np.std(values_unnormalized)+np.std(q_values)
            ...
            if self.gae_lambda is not None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    ## HINT 2: self.gae_lambda is the lambda value in the
                        ## GAE formula
                    delta_i = rews[i]+self.gae_lambda*values[i+1]-values[i]
                    if terminals[i] == 1:
                        advantages[i] = delta_i
                    else:
                        advantages[i] = delta_i + self.gae_lambda*advantages[i+1]
        ...
        if self.standardize_advantages:
            ## OKTODO: standardize the advantages to have a mean of zero
    ...
    def _discounted_return(self, rewards):
        ...
        ret = [ r*self.gamma**i for r,i in zip(rewards,range(len(rewards)))]
        list_of_discounted_returns = np.ones_like(rewards)*ret
    ...
    def _discounted_cumsum(self, rewards):
        ...
        # OKTODO: create `list_of_discounted_returns`
